...
export default function ClassroomView() {
  ...
  useEffect(() => {
    ...
    if (user.classroom !== undefined || typeof user.classroom !== "undefined") {
      setIsLoading(true);
      axios
        .get(`/api/get-classroom-data?classroomID=${classroomID}`)
        .then((res) => {
          console.log(res.data.message);
          const newUser = {
            ...user,
            joinedClassroom: res.data.message,
          };
          setUser(newUser);
    ...
    axios
      ...
      .then((res) => {
        ...
        const newUser = {
          ...user,
          classroom: code,
          joinedClassroom: res.data.joinedClassroom,
        };
        setUser(newUser);
        localStorage.setItem("user", JSON.stringify(newUser));
  ...
  return user.classroom !== undefined && user.joinedClassroom !== undefined ? (
    <Flex minH={"60vh"} align={"center"} justify={"center"} py={12}>
      ...
      >
        <Stack align={"center"} spacing={2}>
          ...
          >
            {user.joinedClassroom.owner.firstName +
              " " +
              user.joinedClassroom.owner.lastName +
              "'s Classroom"}
          </Heading>
          <Divider />
      ...
      >
        ...
        <Stack align={"center"} spacing={2}>
          <Heading
            ...
            color={useColorModeValue("gray.800", "gray.200")}
          >
            You aren't part of a classroom yet!
          </Heading>
          <Text fontSize={"lg"} color={"gray.500"}>
            Ask your teacher for a classroom code (case sensitive)
          </Text>
        </Stack>
        <Stack spacing={4} direction={{ base: "column", md: "row" }} w={"full"}>
          <Input
            onChange={(e) => handleChangeCode(e)}
            placeholder={"7 letter code"}
            value={code}
            color={useColorModeValue("gray.800", "gray.200")}
